...
export default ruleCreator({
    ...
    meta: {
        docs: {
            description: "Disallow using HTML heading elements for settings headings.",
            url: "https://docs.obsidian.md/Plugins/Releasing/Plugin+guidelines#UI+text",
        },
        type: "problem",
        messages: {
            headingEl: "For a consistent UI use `new Setting(containerEl).setName(...).setHeading()` instead of creating HTML heading elements directly.",
        },
        schema: [],
        fixable: "code",
    },
                ...
                context.report({
                    ...
                    fix: (fixer) => {
                        ...
                        if (optionsArg?.type === "ObjectExpression") {
                            const textProp = optionsArg.properties.find((p) => p.type === "Property" &&
                                p.key.name ===
                                    "text");
                            if (textProp?.value.type === "Literal" &&
                                typeof textProp.value.value === "string") {
                                textValue = textProp.value.value;
                            }
                        }
                        const containerEl = context.sourceCode.getText(callee.object);
                        return fixer.replaceText(node, `new Setting(${containerEl}).setName("${textValue}").setHeading()`);
                    },
                });
            },
        };
