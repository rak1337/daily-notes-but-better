...
export function createSentenceCaseReporter(params) {
    const cache = new Map();
    return (node, value) => {
...
function hasPhysicalFilename(value) {
...
export function isEnglishLocalePath(filePath, extensions) {
    ...
    const regex = new RegExp(`(?:^|/)en(?:[._-][^/]+)?(?:/.*)?\\.(?:${extensionPattern})$`, "i");
...
function looksLikePath(text) {
    ...
    if (/[\\/]/.test(text) && /\.[a-z0-9]{1,4}(\b|$)/i.test(text))
        return true;
    return false;
}
// Determines if a string should be skipped from sentence case validation
export function isSkippableString(text) {
    if (!text)
        return true;
...
function normalizeOptions(options) {
    ...
    return { brands, acronyms, ignoreWords, ignoreRegex, mode, enforceCamelCaseLower };
}
// Tests if text matches any of the ignore regex patterns
function shouldIgnoreByRegex(text, regexes) {
...
function collectBrandMatches(sentence, brands) {
    const matches = [];
...
function transformSentence(sentence, opts) {
    ...
    while ((m = tokenRe.exec(sentence))) {
        ...
        if (brandMatches.some((match) => !(end <= match.start || start >= match.end)))
            continue;
        // Skip first token if it's the first alpha token (already handled above)
        if (firstAlpha >= 0 && start <= firstAlpha && firstAlpha < end)
            continue;
        ...
        if (hasHyphen) {
            ...
            const newParts = hyphenParts.map((part) => {
                ...
                if (opts.acronyms.has(partUpper))
                    return partUpper;
                // Preserve brand casing
                if (opts.brands.includes(part))
                    return part;
        ...
        if (opts.brands.includes(token))
            continue;
        // In loose mode, preserve CamelCase unless enforceCamelCaseLower is set
        if (!opts.enforceCamelCaseLower && opts.mode === "loose" && isCamelOrPascal(token))
            continue;
        // Lowercase the token
...
function sentenceCaseSuggestionWithOptions(text, opts) {
    if (!text.trim())
        return text;
    if (shouldIgnoreByRegex(text, opts.ignoreRegex))
        return text;
    const segments = [];
    let lastIndex = 0;
...
export function sentenceCaseSuggestion(text, options) {
...
export function evaluateSentenceCase(text, options) {
