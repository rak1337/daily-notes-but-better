...
import { createSentenceCaseReporter, resolveSentenceCaseConfig, unwrapExpression, } from "./sentenceCaseUtil.js";
...
function isPropertyWithKey(prop, keyName) {
    if (prop.key.type === "Identifier")
        return prop.key.name === keyName;
    if (prop.key.type === "Literal")
        return prop.key.value === keyName;
    return false;
}
...
function inspectCreateElOptions(node, report) {
    ...
    for (const prop of unwrapped.properties) {
        ...
        if (keyName === "attr" && valueNode.type === "ObjectExpression") {
            for (const attrProp of valueNode.properties) {
                ...
                if (attrValue != null)
                    report(attrProp.value, attrValue);
            }
            continue;
        }
...
function visitStatementTree(statement, handleReturn) {
    if (!statement)
        return;
    if (statement.type === "BlockStatement") {
        for (const inner of statement.body)
            visitStatementTree(inner, handleReturn);
        return;
    }
    if (statement.type === "ReturnStatement") {
        ...
        if (str != null)
            handleReturn(statement.argument, str);
        return;
    }
    ...
    if (statement.type === "TryStatement") {
        ...
        if (statement.handler)
            visitStatementTree(statement.handler.body, handleReturn);
        if (statement.finalizer)
            visitStatementTree(statement.finalizer, handleReturn);
    }
}
export default ruleCreator({
    ...
    meta: {
        type: "suggestion",
        docs: {
            description: "Enforce sentence case for UI strings",
    ...
    create(context) {
        ...
        function checkSetAttribute(node) {
        ...
        function checkAddCommand(node) {
            ...
            for (const prop of arg.properties) {
                if (prop.type !== "Property")
                    continue;
                if (!isPropertyWithKey(prop, "name"))
                    continue;
                const str = getStringFromNode(prop.value);
                if (str != null)
        ...
        function checkMethodCall(node) {
            const callee = node.callee;
            // Handle bare calls like createEl(...)
            if (callee.type === "Identifier") {
                if (callee.name === "createEl")
                    checkCreateEl(node);
                if (callee.name === "addCommand")
        ...
        function checkGetDisplayTextReturns(node) {
            if (node.key.type !== "Identifier" || node.key.name !== "getDisplayText")
                return;
            const fn = node.value;
            if (!fn || fn.type !== "FunctionExpression" || !fn.body)
                return;
